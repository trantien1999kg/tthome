<?php
    ...
    class product{
        ...
        public function getAd(){
            ...
            global $ad1,$ad2,$ad3,$lang,$test;

            $ad1 = $this->_d->rawQuery("select id,ten_$lang as ten from #_album where hienthi=1 and type=? order by stt asc, id desc",array('quangcao1'));

            $ad2 = $this->_d->rawQuery("select id,ten_$lang as ten from #_album where hienthi=1 and type=? order by stt asc, id desc",array('quangcao2'));

            $ad3 = $this->_d->rawQuery("select id,ten_$lang as ten from #_album where hienthi=1 and type=? order by stt asc, id desc",array('quangcao3'));
        ...
        public function getMans(){
            ...
            global $type,$ad1,$ad2,$ad3,$url_path,$lang, $items, $paging,$page,$item_list,$items_cat,$items_item;
...

            {
                ...
                $items_cat = $this->_d->rawQuery("SELECT * from #_baiviet_cat where type=? and id_list=? order by stt asc, id desc",array($type,$_GET['id_list']));
...

            {
                ...
                $items_item = $this->_d->rawQuery("SELECT * from #_baiviet_item where type=? and id_cat=? order by stt asc, id desc",array($type,$_GET['id_cat']));
...

            {
                ...
                $items_sub = $this->_d->rawQuery("SELECT * from #_baiviet_sub where type=? and id_item=? order by stt asc, id desc",array($type,$_GET['id_item']));
        ...
        public function getPageList(){
            ...
            if($id){
                 ...
                 $items_cat = $this->_d->rawQuery("SELECT * from #_baiviet_cat where type=? and id_list=? order by stt asc, id desc",array($type,$item['id_list']));



                  $items_item = $this->_d->rawQuery("SELECT * from #_baiviet_item where type=? and id_cat=? order by stt asc, id desc",array($type,$item['id_cat']));
        ...
        public function saveMan(){
            ...
            if($_POST){
                ...
                $checksql = "SELECT group_concat(COLUMN_NAME) as field FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = '".$config['database']['dbname']."' AND TABLE_NAME = 'table_".$this->_table."'";
                
                $columns = $this->_d->rawQuery($checksql);

                $arrcheck = explode(',', $columns[0]['field']);

                foreach ($editor as $k => $v){
                    
                    if(!in_array($k, $arrcheck)){

                        $insertsql = "alter table table_".$this->_table." add ".$k." longtext";

                        $this->_d->rawQuery($insertsql);
                ...
                foreach ($attribute as $k => $v){
                    ...
                    if(!in_array($k, $arrcheck)){
                        ...
                        $insertsql = "alter table table_".$this->_table." add ".$k." varchar(255)";
            ...
            if(!empty($document)){
                ...
                if($id){
                    ...
                    if($document['error']==0){
                        ...
                        $photo = $this->_func->uploadImg($id,"download","",$document,$folder,$this->_com,$table['img-width'],$table['img-height'],$table['img-ratio'],$table['img-b']);

                        

                        $send['download'] = $photo['download'];
                    ...
                    if($document['error']==0){
                        ...
                        $photo = $this->_func->uploadImg(0,"download","",$document,$folder,$this->_com,$table['img-width'],$table['img-height'],$table['img-ratio'],$table['img-b']);
            ...
            if(!empty($file3)){
                ...
                if($id){
                    ...
                    if($file3['error']==0){
                        ...
                        $photo = $this->_func->uploadImg($id,"quangcao","quangcao_thumb",$file3,$folder,$this->_com,$table['img-width'],$table['img-height'],$table['img-ratio'],$table['img-b']);

                        

                        $send['quangcao'] = $photo['quangcao'];

                        $send['quangcao_thumb'] = $photo['quangcao_thumb'];
                    ...
                    if($file3['error']==0){
                        ...
                        $photo = $this->_func->uploadImg(0,"quangcao","quangcao_thumb",$file3,$folder,$this->_com,$table['img-width'],$table['img-height'],$table['img-ratio'],$table['img-b']);
            ...
            if(!empty($file4)){
                ...
                if($id){
                    ...
                    if($file4['error']==0){
                        ...
                        $photo = $this->_func->uploadImg($id,"quangcao1","quangcao1_thumb",$file4,$folder,$this->_com,$table['img-width'],$table['img-height'],$table['img-ratio'],$table['img-b']);

                        

                        $send['quangcao1'] = $photo['quangcao1'];

                        $send['quangcao1_thumb'] = $photo['quangcao1_thumb'];
                    ...
                    if($file4['error']==0){
                        ...
                        $photo = $this->_func->uploadImg(0,"quangcao1","quangcao1_thumb",$file4,$folder,$this->_com,$table['img-width'],$table['img-height'],$table['img-ratio'],$table['img-b']);
            ...
            if($data['count']){
                $send['count']=str_replace(',','',$data['count']);
            }

            if($data['text_search']){
                $send['text_search']=$this->_func->changeSearch($_POST['ten_vi']);
            }
