	...
	if($func->isAjax()){
		...
		switch ($src){
			...
			case 'addCart':
				...
				$result['total']= number_format($cart->getTotalOrder(),0,',','.')."đ";
			...
			case 'updateCart':
				...
				$pcolor = (int)$cart->getPriceProperties('color',$code);

				$psize = (int)$cart->getPriceProperties('size',$code);

				$result['total-product']=$cart->getTotalQuality();

				$result['item-total']=$cart->numbMoney(($cart->getPrice($pid)+$psize+$pcolor)*$qty,' ₫');

				$result['total-price']=$cart->getTotalOrder();

				$result['total']=$cart->numbMoney($cart->getTotalOrder(),'đ');
				...
				$result['html_m']=$cart->getTemplateCheckout_m();
		...
		if($src=='thanh-toan'){
			...
			if(!empty($_POST)){
                ...
                $city = $apiPlace->getPlaceId('id','place_city',$data['id_city'],"id, ten");

                $dist = $apiPlace->getPlaceId('id','place_dist',$data['id_dist'],"id, ten");
				                ...
				                <tr>
				                  ...
				                  if(is_array($_SESSION['cart'])){
									...
									for($i=0;$i<$max;$i++){
				                        ...
				                        $_n_color = $cart->getPropertiesName($color,'ten_'.$lang);

				                        $_n_size = $cart->getPropertiesName($size,'ten_'.$lang);

										$_pcolor = $cart->getPropertiesName($color,'giaban');

										$_psize = $cart->getPropertiesName($size,'giaban');
                ...
                if ($id_insert) {
                    ...
                    if(count($_SESSION['cart'])>0){
                        ...
                        foreach ($_SESSION['cart'] as $k => $v) {
                            ...
                            $data_order['price'] = $product['giaban'];
                    ...
                    if($func->sendMailIndex($row_setting['email'],'Thông báo đơn hàng'.' ['.$order_code.']',$body,$mail_send,null,null)){
                        ...
                        $func->transfer('Cám ơn bạn đã mua hàng tại website của chúng tôi!.', $config_base);
