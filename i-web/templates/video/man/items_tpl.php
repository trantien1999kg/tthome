...
$(document).ready(function() {
    ...
    $('.box-search button').click(function(event) {
        ...
        window.location.href = "index.html?com=video&act=man&type=<?=$_GET['type']?>&keyword=" + keyword;
    });
            ...
            $.confirm({
                ...
                buttons: {
                    success: {
                        ...
                        btnClass: 'btn-blue',
                        action: function(){
                           redirect("index.html?com=video&act=delete&type=<?=$_GET['type']?>&curPage=<?=$_GET['curPage']?>&listid=" + listid);
                        }
                    },
                    cancel: {   
                        text: 'Hủy ngay!',
        ...
        <div class="widget mtop0">
            ...
            <img src="<?=_upload_hinhanh.$items[$i]['photo']?>" width="100" border="0" />
                        ...
                        <td align="center">
                            <label class="stardust-checkbox checkOnOff">
                                <input class="stardust-checkbox__input" data-id="<?=$items[$i]['id']?>"
                                    data-table="table_video" data-type="hienthi"
                                    rel="<?=$items[$i]['hienthi']?>"
                        ...
                        <td align="center">
                            <label class="stardust-checkbox checkOnOff">
                                <input class="stardust-checkbox__input" data-id="<?=$items[$i]['id']?>"
                                    data-table="table_video" data-type="noibat"
                                    rel="<?=$items[$i]['noibat']?>"
                        ...
                        <td class="actBtns">
                            <a class="text-primary" href="index.html?com=video&act=edit&id=<?=$items[$i]['id']?><?php if($_REQUEST['type']!='') echo'&type='. $_REQUEST['type'];?>"
