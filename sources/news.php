<?php  if(!defined('_source')) die("Error");
    ...
    if(empty($id) && empty($idl) && empty($idc) && empty($idi) && empty($ids)){
        ...
        $tintuc=$db->rawQuery("select *,tenkhongdau_$lang as tenkhongdau from table_baiviet where hienthi=1 $combo and type=?{$subWhere} order by stt asc, id desc limit 0,10",array($type));
